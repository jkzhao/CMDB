...
class AssetView(View):
    ...
    @method_decorator(auth.api_auth)  # CBV装饰器
    ...
    def post(self, request, *args, **kwargs):
        ...
        for k, v in config.PLUGINS_DICT.items():
            ...
            response = cls.process(server_obj, server_info, None)  # 插入日志
    ...
    #         return JsonResponse(ret)
    #
    #     # ========》 server_obj是服务器对象 ；用户提交的数据是server_info  《==========
    #     # 硬盘 或 网卡 或 内存
    #     # 硬盘：增删改
    ...
    #     # new_list = ['#1','#4']
    #     #3. 将两个列表转换成集合进行比较，
    #     # 更新是交集['#1'] 删除是差集['#2','#3'] 增加是并集['#4']
    #     # 例子：http://www.cnblogs.com/wupeiqi/articles/5453708.html
    #
    #     #4. # 先增加 ['#4']
    #     """
    #         for i in  ['#4']:
