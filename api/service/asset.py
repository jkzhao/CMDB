...
class HandleNic(object):
    ...
    def process(server_obj, server_info, user_obj):
        ...
        try:
            # 获取数据库中的所有网卡信息
            # server_info，Agent最新汇报的数据 server_info['nic']
...
class HandleDisk(object):
    ...
    def process(server_obj, server_info, user_obj):
        ...
        try:
            ...
            client_disk_dict = disk_info['data'] #汇报过来的数据

            disk_obj_list = models.Disk.objects.filter(server_obj=server_obj) #数据库中已有的数据

            disk_slots = map(lambda x: x, (item.slot for item in disk_obj_list)) #数据库中该主机搜拥有的硬盘符号

            update_list = agorithm.get_intersection(set(client_disk_dict.keys()), set(disk_slots)) #交集
            add_list = agorithm.get_exclude(client_disk_dict.keys(), update_list) #差集，要增加的
            del_list = agorithm.get_exclude(disk_slots, update_list) #差集，要删除的
